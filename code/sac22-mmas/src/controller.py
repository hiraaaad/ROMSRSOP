...
class Parallel_Controller():
            ...
            if len(key_neighbor) > 0:
                ...
                if len(obj_neighbor) == 1:
                    ...
                    if self.problem.type_algorithm in ['DGA', 'RGA']:
                        ...
                        key_edges = [self.problem.D_edge[
                                         (self.machine[idm].current[0].index, x[0], self.machine[idm].current[1], x[1])]
                                     for x in key_neighbor]
                ...
                queue_cut = {'end_node': end_node_all[next_index], 'end_utility': obj_neighbor[next_index, 0],
                             'end_cost': cost_all[next_index],
                             'end_completion': self.time_current + cost_all[next_index],
    ...
    def generate_edges_local(self):
        for idm in np.arange(self.problem.number_machine):
            for ids in np.arange(self.problem.number_demand):
                solution_path = self.machine[idm].solution[ids].solution_path
                id_edges = []
                for i,e in enumerate(solution_path):
                    if i+1 < len(solution_path):
                        id_edge = (e[0].index, solution_path[i+1][0].index, e[1], solution_path[i+1][1])
                        self.edges.add(self.problem.D_edge[id_edge])
